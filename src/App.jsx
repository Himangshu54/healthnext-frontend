...
function Login() {
  ...
  function submit(event) { event.preventDefault(); setError(''); if (!form.identifier || !form.password) return setError(`Enter your ${accountType === 'admin' ? 'Organization Email' : 'Worker ID'} and password.`); setLoading(true); window.setTimeout(async () => { const result = await login(form.identifier, form.password, accountType); if (!result.success) setError(result.error); setLoading(false) }, 350) }
...
function Router() { const { worker } = useAuth(); const [path, setPath] = useState(window.location.pathname); useEffect(() => { const onPop = () => setPath(window.location.pathname); window.addEventListener('popstate', onPop); return () => window.removeEventListener('popstate', onPop) }, []); useEffect(() => { const protectedPath = path !== '/login'; if (!worker && protectedPath) navigate('/login'); if (worker && path === '/login') navigate(worker.accountType === 'admin' ? '/admin' : '/') }, [path, worker]); if (!worker) return <Login />; const pages = worker.accountType === 'admin' ? { '/admin': <AdminDashboard navigate={navigate} />, '/admin/users': <UserManagement />, '/admin/patients': <PatientDetails />, '/profile': <Profile /> } : { '/': <EmployeeDashboard />, '/create-session': <CreateSession />, '/patients': <EmployeePatientsTable />, '/generate-report': <PatientGenerateReport />, '/report-analysis': <PatientAnalysis />, '/profile': <Profile /> }; return <Shell path={path}>{pages[path] || (worker.accountType === 'admin' ? <AdminDashboard navigate={navigate} /> : <EmployeeDashboard />)}</Shell> }
